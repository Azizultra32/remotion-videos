...
Engine-lock: chat-driven `Edit`/`Write` on engine paths is blocked by the `PreToolUse` hook. CRITICAL CARVE-OUTS — the hook does NOT block writes to `projects/<stem>/**` (including `custom-elements/*.tsx`), `src/compositions/**`, `brands/**`, `out/**`, or `.current-project`. These are free-write zones and do not need `ENGINE_UNLOCK=1`. The reply-with-ceremony script *"requires ENGINE_UNLOCK=1; set ENGINE_UNLOCK=1 in your shell and restart the editor, then re-issue"* applies ONLY when the target path is genuinely engine infrastructure (`src/hooks/**`, `src/lib/**`, `editor/**`, `scripts/**`, `docs/**`, `package.json`, root config files, etc.). Check the path before reciting the ceremony.
