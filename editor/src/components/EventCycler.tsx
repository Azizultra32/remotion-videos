...
export const EventCycler = () => {
  ...
  return (
    <div
      ...
      title="Drag the yellow lines on the waveform above to move events. Select + press Delete to remove."
    >
      <button
      ...
      {events.map((t, i) => (
        <button
          ...
          title={`Event ${i + 1} at ${t.toFixed(2)}s — click to seek. Drag the yellow line on the waveform to move.`}
        >
          {`EVT ${i + 1}  ${fmtTime(t)}`}
        </button>
      ))}
