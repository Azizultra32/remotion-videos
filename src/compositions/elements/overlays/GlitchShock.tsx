...
import { findLatestTriggerWithinTrail, selectTriggerTimes } from "../triggerRuntime";
...
const Renderer: React.FC<ElementRendererProps<Props>> = ({ element, ctx }) => {
  ...
  const glStateRef = useRef<FullscreenShaderState<
    "uResolution" | "uAges" | "uRects" | "uActiveCount" | "uTrail" | "uColor"
  > | null>(null);
  ...
  const pack = useMemo(() => {
    const lastBeat = findLatestTriggerWithinTrail({
      triggerTimes,
      tSec,
      trailSec,
    });
    if (lastBeat == null) {
      return { ages: new Float32Array(8).fill(-1), rects: new Float32Array(32), count: 0 };
    }
  ...
  }, [triggerTimes, tSec, trailSec, rectCount, maxOffset]);
  ...
  useEffect(() => {
    ...
    const state = createFullscreenShaderState(gl, {
      fragmentSource: FRAG,
      label: "GlitchShock",
      uniformNames: ["uResolution", "uAges", "uRects", "uActiveCount", "uTrail", "uColor"] as const,
      vertexSource: VERT,
    });
    if (!state) return;
...
export const GlitchShockModule: ElementModule<Props> = {
  ...
  label: "Glitch Shock",
  description:
    "Beat-triggered datamosh rectangles with RGB-offset tears. Up to 8 rects per beat, decays in 120ms.",
